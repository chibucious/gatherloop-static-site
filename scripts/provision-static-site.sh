...
# NOTE (added after initial deploy): this script generates a new random
# storage account name every time it's run, so re-running it will create a
# SECOND, differently-named storage account rather than updating the
# existing one. That's intentional for spinning up a fresh environment, but
# worth flagging so it's not mistaken for an idempotent "redeploy" script.
# The actual live deployed instance for this project is: gatherloopsite18025
